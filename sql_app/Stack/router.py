...
from sql_app.Employment.models import emp_stack, Emp
from sql_app.Employment.schema import EmpStackSelect
...
@stack_router.get("/api/v1/stack/id/{id}",response_model=List[CompanySelect])
async def stack_select(
        id : int,
        db : Database = Depends(get_db_conn)
):
    query = select([emp_stack, company]).where(emp_stack.columns.stack_fk == id).where(emp_stack.columns.emp_fk == company.columns.emp_fk)
